...
from .config import HOST, PORT_R3, INPUT_DIR
...
def receive_packet(connection, max_buffer_size):
    ...
    write_to_file("./output/recieved_by_router3.txt", decoded_packet)
...
def start_server():
    ...
    try:
        ...
        sys.exit()
    soc.listen(5)
    print("Router 3 listening on", port)
    ...
    while True:
        ...
        print("Router 3 connected with " + ip + ":" + port_str)
        try:
            t = Thread(
...
def processing_thread(connection, ip, port,
    ...
    while True:
        ...
        if send_interface is None and not is_last_hop:
            send_interface = default_gateway_port  # "d" (toward Router 2)
            if send_interface == HOST:
                is_last_hop = True

        if is_last_hop:
            if ttl > 0:
                print("Router 3 OUT:", payload)
                write_to_file("./output/out_router3.txt", payload)
            else:
                packet_with_zero = ",".join([sourceIP, destinationIP, payload, "0"])
                print("Router 3 DISCARD:", packet_with_zero)
                write_to_file("./output/discarded_by_router3.txt", packet_with_zero)
            continue
        ...
        if send_interface in ("d", str(PORT_R3)):
            print("Router 3 forwarding", new_packet, "to Router 2")
            write_to_file("./output/sent_by_router3.txt", new_packet, "2")
            # only neighbor is Router 2; use the same connection back
            connection.send(new_packet.encode())
        else:
            print("Router 3 DISCARD:", new_packet)
            write_to_file("./output/discarded_by_router3.txt", new_packet)
