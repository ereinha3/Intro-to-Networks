...
from .config import HOST, PORT_R6, INPUT_DIR
...
def receive_packet(connection, max_buffer_size):
    ...
    print("Router 6 recieved packet", decoded_packet)
    write_to_file("./output/recieved_by_router6.txt", decoded_packet)
...
def start_server():
    ...
    try:
        ...
        sys.exit()
    soc.listen(5)
    print("Router 6 listening on", port)
    ...
    while True:
        ...
        print("Router 6 connected with " + ip + ":" + port_str)
        try:
            t = Thread(
...
def processing_thread(connection, ip, port,
    ...
    while True:
        ...
        for row in forwarding_table_with_range:
            ...
            if destinationIP_int >= min_ip and destinationIP_int <= max_ip:
                ...
                if iface == HOST:
                    ...
                    send_interface = iface  # "f" toward Router 4
                break

        if send_interface is None and not is_last_hop:
            send_interface = default_gateway_port  # "f"
            if send_interface == HOST:
                is_last_hop = True

        if is_last_hop:
            if ttl > 0:
                print("Router 6 OUT:", payload)
                write_to_file("./output/out_router6.txt", payload)
            else:
                packet_with_zero = ",".join([sourceIP, destinationIP, payload, "0"])
                print("Router 6 DISCARD:", packet_with_zero)
                write_to_file("./output/discarded_by_router6.txt", packet_with_zero)
            continue
        ...
        if send_interface in ("f", "8006"):
            print("Router 6 forwarding", new_packet, "to Router 4")
            write_to_file("./output/sent_by_router6.txt", new_packet, "4")
            connection.send(new_packet.encode())   # only neighbor is Router 4
        else:
            print("Router 6 DISCARD:", new_packet)
            write_to_file("./output/discarded_by_router6.txt", new_packet)
