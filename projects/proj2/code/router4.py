...
from .config import HOST, PORT_R4, PORT_R5, PORT_R6, INPUT_DIR
...
def receive_packet(connection, max_buffer_size):
    ...
    print("Router 4 recieved packet", decoded_packet)
    write_to_file("./output/recieved_by_router4.txt", decoded_packet)
...
def start_server():
    host = HOST
    port = PORT_R4  # Router 4 listens here (neighbors 1 and 2 connect)
    ...
    try:
        ...
        sys.exit()
    soc.listen(5)
    print("Router 4 listening on", port)

    forwarding_table = read_csv("router_4_table.csv")
    default_gateway_port = find_default_gateway(forwarding_table)
    forwarding_table_with_range = generate_forwarding_table_with_range(forwarding_table)

    # outgoing connections from Router 4 to Routers 5 and 6
    soc_to_r5 = create_socket(host, PORT_R5)
    soc_to_r6 = create_socket(host, PORT_R6)

    while True:
        connection, address = soc.accept()
        ip, port_str = str(address[0]), str(address[1])
        print("Router 4 connected with " + ip + ":" + port_str)
        try:
            t = Thread(
...
def processing_thread(connection, ip, port,
    ...
    while True:
        ...
        if send_interface is None and not is_last_hop:
            send_interface = default_gateway_port  # PORT_R5 by table
            if send_interface == HOST:
                is_last_hop = True

        if is_last_hop:
            if ttl > 0:
                print("Router 4 OUT:", payload)
                write_to_file("./output/out_router4.txt", payload)
            else:
                packet_with_zero = ",".join([sourceIP, destinationIP, payload, "0"])
                print("Router 4 DISCARD:", packet_with_zero)
                write_to_file("./output/discarded_by_router4.txt", packet_with_zero)
            continue
        ...
        if send_interface == str(PORT_R5):
            print("Router 4 forwarding", new_packet, "to Router 5")
            write_to_file("./output/sent_by_router4.txt", new_packet, "5")
            soc_to_r5.send(new_packet.encode())
        elif send_interface == str(PORT_R6):
            print("Router 4 forwarding", new_packet, "to Router 6")
            write_to_file("./output/sent_by_router4.txt", new_packet, "6")
            soc_to_r6.send(new_packet.encode())
        else:
            print("Router 4 DISCARD:", new_packet)
            write_to_file("./output/discarded_by_router4.txt", new_packet)
